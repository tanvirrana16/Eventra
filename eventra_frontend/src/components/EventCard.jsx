...
export default function EventCard({ event, onViewDetails, showStatusBadge }) {
  const { title, dateText, dateBadge, time, venue, image, organizer, rating } = event;
  ...
  return (
    <div className="flex-none w-72 sm:w-80 md:w-full bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 hover:border-gray-200 transition-all duration-300 transform hover:-translate-y-1.5 flex flex-col justify-between group font-outfit select-none">
      ...
      <div className="relative aspect-[16/10] overflow-hidden shrink-0">
        ...
        {/* Status Badge (top-right corner, e.g. "Upcoming", "Registration Open") */}
        {showStatusBadge && event.status && (
          <span className="absolute top-3 right-3 bg-emerald-500 text-white font-extrabold text-[10px] tracking-wide uppercase px-3 py-1 rounded-full shadow-md z-20">
            {event.status}
          </span>
        )}
      ...
      <div className="p-5 flex-grow text-left space-y-4">

        {/* Title */}
        <h3 className="text-lg font-bold text-gray-900 line-clamp-2 group-hover:text-[#2E6F40] transition-colors duration-300 leading-snug">
          {title}
        </h3>
